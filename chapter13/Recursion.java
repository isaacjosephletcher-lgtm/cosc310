...
public class Recursion {
    ...
    // otherwise returns the index of first occurrence
    ...
    public static int count_r(int lis[]) {
        if (lis.length == 0)
            ...
            return 1 + count_r(java.util.Arrays.copyOfRange(lis, 1, lis.length));
    }

    public static int count_r2(int lis[]) {
        return countHelper(lis, 0);
    }

    public static int countHelper(int lis[], int i) {
        if (lis.length == 0) 
            return i;
        else
            return countHelper(java.util.Arrays.copyOfRange(lis, 1, lis.length), i + 1);
    }

    public static void main(String[] args) {
        System.out.println(sum(new int[]{1, 7, 8, 3}));
    }
}
